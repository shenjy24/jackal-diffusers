...
def generate_image(request_id, model, positive_prompt, negative_prompt):
    ...
    image = pipe(prompt=positive_prompt, negative_prompt=negative_prompt, num_inference_steps=8, guidance_scale=8.0,
                  lcm_origin_steps=50, output_type="pil", weight=512, height=512, callback=callback).images[0]
    return image
...
if __name__ == '__main__':
    ...
    img = paint_image(request_id=None, model="SimianLuo/LCM_Dreamshaper_v7", positive_prompt=pp, negative_prompt=np,
                     upscale_model="stabilityai/stable-diffusion-x4-upscaler")
    img.save(f"imgage_lcm.png")
